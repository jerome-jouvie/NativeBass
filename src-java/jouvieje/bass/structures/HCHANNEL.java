...
public class HCHANNEL extends Pointer {
	...
	public static HCHANNEL asHCHANNEL(Pointer pointer) {
		...
		return new HCHANNEL(address);
	}
	/**
	 * Allocate and initialize a new <code>HCHANNEL[]</code>.<br>
	 * @param length length of the array returned.
	 */
	public static HCHANNEL[] allocate(int length) {
		if(length <= 0) {
			 return null;
		}
		...
		final long sizeOf = StructureJNI.HCHANNEL_SIZEOF();
		HCHANNEL[] array = new HCHANNEL[length];
		for(int i = 0; i < length; i++) {
			array[i] = new HCHANNEL(first + i * sizeOf);
		}
		return array;
	}
	 ...
	 * <pre><code>  HCHANNEL obj = HCHANNEL.allocate();</code></pre>
	 */
	public HCHANNEL() {
		super();
	}
